...
function driver(data) {
    const dateOfBirth = data[3].split('-')
    ...
    const monthOfBirth = monthNumbers[dateOfBirth[1].slice(0, 3)]
    const monthCode = ((data[4] === "F" ? monthOfBirth + 50 : monthOfBirth) + "").padStart(2, 0)

    return data[2].slice(0, 5).toUpperCase().padEnd(5, 9)
        + dateOfBirth[2][2]
        + monthCode + dateOfBirth[0]
        + dateOfBirth[2][3] + data[0].slice(0, 1) + (data[1].slice(0, 1) || '9') + '9AA'
}