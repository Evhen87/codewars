...
function workOnStrings(a,b){
    ...
    const objA = stringsToObj(a)
    const objB = stringsToObj(b)

    function resString(str, obj){
        let resStr = ''
        for (let char of str) {
            if (obj[char.toLowerCase()]) {
                if (obj[char.toLowerCase()] % 2 !== 0) {
                    resStr += char.toLowerCase() === char ? char.toUpperCase() : char.toLowerCase()
                } else {resStr += char}
            } else {
                resStr += char
            }
    ...
    return resString(a, objB) + resString(b, objA)
}
