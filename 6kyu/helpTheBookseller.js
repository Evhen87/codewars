...
function stockList(books, categories) {
    ...
    const obj = {}
    categories.forEach((element) => obj[element] = 0)

    books.forEach((element) => element.split('')[0] in obj ? obj[element.split('')[0]] += +element.replace(/[^\d]/g, '') : obj)

    return categories.map((element) => element in obj ?
        `(${element} : ${obj[element]})` :
        `${element} : 0`).join(' - ');
}