...
function alphabetWar(fight) {
    ...
    for (let i = 0; i < fightArr.length; i++) {
        const currentLetter = fightArr[i]
        const nextLetter = fightArr[i + 1]
        if (currentLetter === '*') {
            fightArr[i - 1] = ''
            if (nextLetter !== '*') {
                fightArr[i + 1] = ''
    ...
    for (let i = 0; i < fightArr.length; i++) {
        if (rightObj[fightArr[i]]) {
            right += rightObj[fightArr[i]]
        }
        if (leftObj[fightArr[i]]) {
            left += leftObj[fightArr[i]]
        }
    }
