...
function looseChange(cents){
    ...
    const roundedCents = Math.floor(cents)

    coins.Quarters = Math.floor(roundedCents/25)
    coins.Dimes = Math.floor(roundedCents % 25 / 10)
    coins.Nickels = Math.floor(roundedCents % 25 % 10 / 5)
    coins.Pennies = Math.floor(roundedCents % 25 % 10 % 5)
    return coins
}